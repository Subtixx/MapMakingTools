...
public class PacketBiomeUpdate extends AbstractClientMessage {
	...
	public void read(PacketBuffer packetbuffer) throws IOException {
		...
		this.biome = Biome.REGISTRY.getObjectById(packetbuffer.readInt());
	}
	@Override
	public void write(PacketBuffer packetbuffer) throws IOException {
		packetbuffer.writeBlockPos(this.pos1);
		packetbuffer.writeBlockPos(this.pos2);
		packetbuffer.writeInt(Biome.REGISTRY.getIDForObject(this.biome));
	}
	...
	public void process(EntityPlayer player, Side side) {
		...
		for(BlockPos pos : positions) {
			...
			biomes[((pos.getZ() & 0xF) << 4 | pos.getX() & 0xF)] = (byte)Biome.REGISTRY.getIDForObject(this.biome);
