...
public class BabyMonsterClientFilter extends IFilterClientSpawner {
	...
	public void initGui(IGuiFilter gui) {
		...
		if(tile instanceof TileEntityMobSpawner) {
			...
			if(SpawnerUtil.isBabyMonster(spawner.getSpawnerBaseLogic(), minecartIndex)) {
	...
	public void actionPerformed(IGuiFilter gui, GuiButton button) {
		...
		if (button.enabled) {
            switch (button.id) {
                case 0:
                	PacketDispatcher.sendToServer(new PacketBabyMonster(gui.getBlockPos(), this.btn_covert.getData() != 0, minecartIndex));
	...
	public boolean showErrorIcon(IGuiFilter gui) {
        ...
        String mobId = SpawnerUtil.getMobId(spawner.getSpawnerBaseLogic(), -1);
		return !(mobId.equals("Zombie") || mobId.equals("PigZombie"));
	}
