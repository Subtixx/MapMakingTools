...
public class PotionAttribute extends IItemAttribute {
	...
	public void onItemCreation(ItemStack stack, int data) {
		if(!Strings.isNullOrEmpty(this.level) && !Strings.isNullOrEmpty(this.duration) && selected != -1 && data == 0) {
			if(NumberParse.isInteger(this.level)) {
				...
				Potion potion = Potion.getPotionById(PotionList.getPotionId(PotionList.getCustomId(selected)));
		...
		if(selectedDelete != -1 && data == 1) {
			if(stack.hasTagCompound() && stack.getTagCompound().hasKey("CustomPotionEffects", 9)) {
		        NBTTagList nbttaglist = stack.getTagCompound().getTagList("CustomPotionEffects", 10);
		        nbttaglist.removeTag(selectedDelete);
			}
			else {
