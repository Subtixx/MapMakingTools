...
public class GuiTabSelect extends GuiButton {
    ...
    public void drawButton(Minecraft mc, int i, int j) {
        if(this.visible) {
            ...
            this.drawTexturedModalRect(this.xPosition + (isSelected && type == ButtonType.RIGHT ? -2 : 0), this.yPosition, (isSelected ? 30 : 0), 28 * (type == ButtonType.LEFT ? 1 : 0), difference, 27 * (type == ButtonType.LEFT ? 2 : 1));//top left
            GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
            String path = this.filter.getIconPath();
            mc.getTextureManager().bindTexture(ResourceReference.getTexture(this.filter.getIconPath()));
            drawModalRectWithCustomSizedTexture(this.xPosition + 5 + (type == ButtonType.LEFT ? 3 : 0), this.yPosition + 5, 0, 0, 16, 16, 16, 16);
