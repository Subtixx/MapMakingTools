...
public class SpawnerUtil {
    ...
    private static Field randomMinecartListField = ReflectionHelper.getField(MobSpawnerBaseLogic.class, 1);
	...
	public static String getMobId(MobSpawnerBaseLogic spawnerLogic, int minecartIndex) {
		if(minecartIndex == -1)
        {
            return ReflectionHelper.getField(randomMinecartField, WeightedSpawnerEntity.class, spawnerLogic).getNbt().getString("id");
        }
			//return ReflectionHelper.getField(mobIdField, String.class, spawnerLogic);
		else {
			WeightedSpawnerEntity randomMinecart = ReflectionHelper.getField(randomMinecartField, WeightedSpawnerEntity.class, spawnerLogic);
	...
	public static void setMobId(MobSpawnerBaseLogic spawnerLogic, String mobId, int minecartIndex) {
		if(minecartIndex == -1) {
            ReflectionHelper.getField(randomMinecartField, WeightedSpawnerEntity.class, spawnerLogic).getNbt().setString("id", mobId);
			...
			WeightedSpawnerEntity randomMinecart = (WeightedSpawnerEntity)getRandomMinecarts(spawnerLogic).get(minecartIndex);
			NBTTagCompound data = randomMinecart.getNbt();
			data.setString("Type", mobId);
			WeightedSpawnerEntity newRandomMinecart = new WeightedSpawnerEntity(data);
			getRandomMinecarts(spawnerLogic).set(minecartIndex, newRandomMinecart);
			spawnerLogic.setNextSpawnData(newRandomMinecart);
		}
	}
	...
	public static void setMobArmor(MobSpawnerBaseLogic spawnerLogic, ItemStack helment, ItemStack chestplate, ItemStack leggings, ItemStack boots, ItemStack heldItem, ItemStack offhandItem, int minecartIndex) {
		WeightedSpawnerEntity randomMinecart = ReflectionHelper.getField(randomMinecartField, WeightedSpawnerEntity.class, spawnerLogic);
		NBTTagCompound tag = getMinecartProperties(randomMinecart);
        ...
        // Offhand
        nbttagcompound1 = new NBTTagCompound();
        if(offhandItem != null) offhandItem.writeToNBT(nbttagcompound1);
        nbttaglist2.appendTag(nbttagcompound1);
		...
		tag.setTag("HandItems", nbttaglist2);
	...
	public static ItemStack[] getMobArmor(MobSpawnerBaseLogic spawnerLogic, int minecartIndex) {
		...
		if (tag.hasKey("HandItems")) {
            ...
            equipment[0] = ItemStack.loadItemStackFromNBT(nbttaglist.getCompoundTagAt(0));
            equipment[1] = ItemStack.loadItemStackFromNBT(nbttaglist.getCompoundTagAt(1));
        }
		...
		if (tag.hasKey("ArmorItems")) {
		    ...
		    for (int i = 0; i < equipment.length-2; ++i)
		    	equipment[i+2] = ItemStack.loadItemStackFromNBT(nbttaglist.getCompoundTagAt(i));
		}
		return equipment;
	}
	...
	public static void setPosition(MobSpawnerBaseLogic spawnerLogic, double x, double y, double z, int minecartIndex) {
		...
		//	tag.removeTag("Pos");
		//}
		tag.setTag("Pos", newDoubleNBTList(x, y, z));
		spawnerLogic.setNextSpawnData(randomMinecart);
	}
	...
	public static void setVelocity(MobSpawnerBaseLogic spawnerLogic, double xMotion, double yMotion, double zMotion, int minecartIndex) {
        WeightedSpawnerEntity randomMinecart = ReflectionHelper.getField(randomMinecartField, WeightedSpawnerEntity.class, spawnerLogic);
		NBTTagCompound tag = getMinecartProperties(randomMinecart);
		tag.setTag("Motion", newDoubleNBTList(xMotion, yMotion, zMotion));
		spawnerLogic.setNextSpawnData(randomMinecart);
	}
	...
	public static void confirmHasRandomMinecart(MobSpawnerBaseLogic spawnerLogic) {
		List<WeightedSpawnerEntity> minecarts = getRandomMinecarts(spawnerLogic);
	...
	public static List<WeightedSpawnerEntity> getRandomMinecarts(MobSpawnerBaseLogic spawnerLogic) {
	...
	public static NBTTagCompound getMinecartProperties(WeightedSpawnerEntity minecart) {
	    /*if(!minecart.getNbt().hasKey("Properties"))
	        minecart.getNbt().setTag("Properties", new NBTTagCompound());*/
		return minecart.getNbt();//.getCompoundTag("Properties");
	}
	
	public static String getMinecartType(WeightedSpawnerEntity minecart) {
		return minecart.getNbt().getString("Type");
	}
